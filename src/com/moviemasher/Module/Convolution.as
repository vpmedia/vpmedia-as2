...
/** Effect module adjusts pixels of underlying video and effect tracks, based on a matrix of integers.

*/

class com.moviemasher.Module.Convolution extends com.moviemasher.Module.Module
{
	...
	function applyEffect(key : String, values : Object, mc : MovieClip, done : Number, size: Object) : Object
	{
		...
		var apply_values = {};
		copyValues(apply_values, ['matrix','bias','divisor'], values, config, __defaults);
		
		
		var values_index = 'apply_filter_values_' + config.id;
		if ((mc[values_index] == undefined) || (! _global.com.moviemasher.utils.Object.equals(apply_values, mc[values_index]))) 
		{
			apply_status.changed = true;
			mc[values_index] = values;
			var clip_name = values_index + '_mc';
			var matrix : Array;
			matrix = apply_values.matrix.split(',');
			var z = matrix.length;
			var matrix_dim = Math.sqrt(z);
			for (var i = 0; i < z; i++)
			{
				matrix[i] = Number(matrix[i]);
			}
			var divisor = apply_values.divisor;
			var bias = apply_values.bias;
			
			if (! bias) bias = 0;
			if (! divisor) divisor = _global.com.moviemasher.Utility.ArrayUtility.sum(matrix);
			mc.applyFilter[key] = new flash.filters.ConvolutionFilter(matrix_dim, matrix_dim, matrix, divisor, bias);
		}
		return apply_status;
	}
	private static var __defaults : Object = {matrix: '-10,10,-10,10,10,10,-10,10,-10', bias: 0, divisor: 0};
	
	private function Convolution()
	{
