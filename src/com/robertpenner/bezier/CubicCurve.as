...
class com.robertpenner.bezier.CubicCurve {
	...
	private function bezierSplit (p0:Object, p1:Object, p2:Object, p3:Object):Object {
		...
		return {b0:{a:p0, b:p01, c:p02, d:p03}, b1:{a:p03, b:p13, c:p23, d:p3}};
	}
	...
	public function drawBezier (x1:Number, y1:Number, x2:Number, y2:Number, x3:Number, y3:Number, x4:Number, y4:Number, tolerance:Number):Void {
		this.drawBezierPts ({x:x1, y:y1}, {x:x2, y:y2}, {x:x3, y:y3}, {x:x4, y:y4}, tolerance);
	}
	private function curveToCubic (x1:Number, y1:Number, x2:Number, y2:Number, x3:Number, y3:Number, tolerance:Number):Void {
		if (tolerance == undefined) {
			tolerance = 5;
		}
		this._cBez ({x:this._xpen, y:this._ypen}, {x:x1, y:y1}, {x:x2, y:y2}, {x:x3, y:y3}, tolerance * tolerance);
	}
	private function curveToCubicPts (p1:Object, p2:Object, p3:Object, tolerance:Number):Void {
		if (tolerance == undefined) {
			tolerance = 5;
		}
		this._cBez ({x:this._xpen, y:this._ypen}, p1, p2, p3, tolerance * tolerance);
	}
