class com.vpmedia.kXmlDataObj extends XML {
	...
	function _addObject (refNode, objType) {
		...
		for (var i = 0; i < refNode.childNodes.length; i++) {
			if (refNode.childNodes[i].nodeName == "var") {
			...
			if (refNode.childNodes[i].nodeName == "array") {
				var _tmpKey = refNode.childNodes[i].attributes.name;
				if (_tmpKey != "") {
