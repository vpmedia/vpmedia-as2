/**
	...
	/**
	        ...
	        */
	public static function md5 (s:String, b64pad:String, chrsz:Number):String {
		if (b64pad != undefined)
		{
			CustomString.b64pad = b64pad;
		}
		if (chrsz != undefined && chrsz == 8 || chrsz == 16)
		{
			CustomString.chrsz = chrsz;
		}
		return com.vpmedia.CustomString.hex_md5 (s);
	}
	/**
	...
	private static function hex_md5 (s:String):String {
		return com.vpmedia.CustomString.binl2hex (core_md5 (str2binl (s), s.length * com.vpmedia.CustomString.chrsz));
	}
	private static function b64_md5 (s:String):String {
		return com.vpmedia.CustomString.binl2b64 (core_md5 (str2binl (s), s.length * com.vpmedia.CustomString.chrsz));
	}
	private static function str_md5 (s:String):String {
		return com.vpmedia.CustomString.binl2str (core_md5 (str2binl (s), s.length * com.vpmedia.CustomString.chrsz));
	}
	private static function hex_hmac_md5 (key:String, data:String):String {
		return com.vpmedia.CustomString.binl2hex (core_hmac_md5 (key, data));
	}
	private static function b64_hmac_md5 (key:String, data:String):String {
		return com.vpmedia.CustomString.binl2b64 (core_hmac_md5 (key, data));
	}
	private static function str_hmac_md5 (key:String, data:String):String {
		return com.vpmedia.CustomString.binl2str (core_hmac_md5 (key, data));
	}
	/**
	        * These functions implement the four basic operations the algorithm uses.
	        */
	private static function md5_cmn (q:Number, a:Number, b:Number, x:Number, s:Number, t:Number):Number {
		return com.vpmedia.CustomString.safe_add (bit_rol (safe_add (safe_add (a, q), safe_add (x, t)), s), b);
	}
	private static function md5_ff (a:Number, b:Number, c:Number, d:Number, x:Number, s:Number, t:Number):Number {
		return com.vpmedia.CustomString.md5_cmn ((b & c) | ((~b) & d), a, b, x, s, t);
	}
	private static function md5_gg (a:Number, b:Number, c:Number, d:Number, x:Number, s:Number, t:Number):Number {
		return com.vpmedia.CustomString.md5_cmn ((b & d) | (c & (~d)), a, b, x, s, t);
	}
	private static function md5_hh (a:Number, b:Number, c:Number, d:Number, x:Number, s:Number, t:Number):Number {
		return com.vpmedia.CustomString.md5_cmn (b ^ c ^ d, a, b, x, s, t);
	}
	private static function md5_ii (a:Number, b:Number, c:Number, d:Number, x:Number, s:Number, t:Number):Number {
		return com.vpmedia.CustomString.md5_cmn (c ^ (b | (~d)), a, b, x, s, t);
	}
	/**
	...
	private static function core_hmac_md5 (key:String, data:String):Array {
		...
		if (bkey.length > 16)
		{
			bkey = core_md5 (bkey, key.length * com.vpmedia.CustomString.chrsz);
		}
		...
		var hash:Array = new Array (core_md5 (ipad.concat (str2binl (data)), 512 + data.length * com.vpmedia.CustomString.chrsz));
		return com.vpmedia.CustomString.core_md5 (opad.concat (hash), 512 + 128);
	}
	/**
	...
	/**
	        ...
	        * If CustomString.chrsz is ASCII, characters >255 have their hi-byte silently ignored.
	        */
	private static function str2binl (str:String):Array {
		var bin:Array = new Array ();
		var mask:Number = (1 << com.vpmedia.CustomString.chrsz) - 1;
		for (var i:Number = 0; i < str.length * com.vpmedia.CustomString.chrsz; i += com.vpmedia.CustomString.chrsz)
		{
			bin[i >> 5] |= (str.charCodeAt (i / com.vpmedia.CustomString.chrsz) & mask) << (i % 32);
		}
		return bin;
	}
	/**
	...
	private static function binl2str (bin:Array):String {
		...
		var mask:Number = (1 << com.vpmedia.CustomString.chrsz) - 1;
		for (var i:Number = 0; i < bin.length * 32; i += com.vpmedia.CustomString.chrsz)
		{
	...
	private static function binl2b64 (binarray:Array):String {
		...
		{
			var triplet:Object = (((binarray[i >> 2] >> 8 * (i % 4)) & 0xFF) << 16) | (((binarray[i + 1 >> 2] >> 8 * ((i + 1) % 4)) & 0xFF) << 8) | ((binarray[i + 2 >> 2] >> 8 * ((i + 2) % 4)) & 0xFF);
			for (var j:Number = 0; j < 4; j++)
			{
				if (i * 8 + j * 6 > binarray.length * 32)
				{
					str += com.vpmedia.CustomString.b64pad;
				}
				else
				{
