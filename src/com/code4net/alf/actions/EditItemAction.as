...
class com.code4net.alf.actions.EditItemAction implements ICommand{
	public function execute(params:Object) {
		var item = FlashLibrary.getInstance().selectedItems[0];
		
		if (item[ItemHelper.ITEM_TYPE == "screen"]) {
			LibraryHelper.editScreen(item[ItemHelper.NAME]);
		} else {
			LibraryHelper.editItem(item[ItemHelper.NAME]);
		}
	}
}