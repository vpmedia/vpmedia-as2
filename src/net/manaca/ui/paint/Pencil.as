...
import net.manaca.ui.paint.PencilRecord;
import net.manaca.ui.paint.Drawing;

/**
 * 铅笔绘画工具
 * @author Wersling
 * @version 1.0, 2006-4-17
 */
class net.manaca.ui.paint.Pencil extends BObject implements Drawing{
	private var className : String = "net.manaca.ui.paint.Pencil";
	...
	private var _now_PancilRecord : PencilRecord;
	/**
	 * 构造一个铅笔工具
	 * @param mc 指定画板
	 */
	public function Pencil(mc:MovieClip) {
		super();
		if(mc != undefined){
			_paint_mc = mc;
		}else{
	...
	public function startDraw(x : Number, y : Number,mcName:String) : Array {
		_isdraw = true;
		if(mcName ==undefined) {
			var _mcName = "Pencil_"+PencilRecord.getId();
		}else{
			var _mcName = mcName;
		}
		...
		_now_PancilRecord = new PencilRecord(_mcName,Thickness, RGB, Alpha);
		_record.push(_now_PancilRecord);
		return _now_PancilRecord.pushLine(x,y);
	}
	...
	public function draw(x : Number, y : Number) : Array {
		if (_isdraw) {
			_paint_mc[_now_PancilRecord.getName()].lineTo(x, y);
			return _now_PancilRecord.pushLine(x,y);
		}
	}
	...
	public function endDraw(x : Number, y : Number) : Array {
		_isdraw = false;
		return _now_PancilRecord.endRecord();
	}
