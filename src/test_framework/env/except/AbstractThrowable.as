...
import test_framework.env.except.StackTraceElement;
import test_framework.env.except.Throwable;
import test_framework.env.except.ThrowableStringifier;
 ...
 *
 * @author Simon Wacker
 * @see env.except.Throwable
 */
class test_framework.env.except.AbstractThrowable extends Error {
	...
	private static function getLogger(Void):Logger {
		if (!logger) {
			logger = LogManager.getLogger("env.except.Throwable");
		}
		return logger;
	}
	 ...
	 *   } catch (e:test_framework.env.except .Throwable) {
	 ...
	 * @throws env.except.IllegalArgumentException if the passed-in
	 * {@code newCause} is {@code null} or {@code undefined}
	 * @throws env.except.IllegalStateException if the cause has
