...
import test_framework.env.event.broadcaster.EventBroadcaster;
import test_framework.env.event.broadcaster.EventBroadcasterFactory;
 ...
 */
class test_framework.env.event.broadcaster.DynamicEventBroadcasterFactory implements EventBroadcasterFactory {
	...
	public function setEventBroadcasterClass(clazz:Function) {
		if (!ClassUtil.isImplementationOf(clazz, EventBroadcaster)){
			...
			throw new IllegalArgumentException("Argument 'clazz' [" + clazz + ":" + className + "] is not an implementation of the 'env.event.EventBroadcaster' interface.", this, arguments);
		}
		this.clazz = clazz;
	}
