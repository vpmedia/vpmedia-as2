 ...
 * @author Simon Wacker
 */
class test_framework.env.reflect.algorithm.ClassAlgorithm {
		...
		for (i in p) {
			...
			if (typeof(f) == "function") {
				if (v(f)) {
					...
					// e.g. core.BasicClass is stored in _global with name org_as2lib_core_BasicClass
	 ...
	 * class's path (namespace) as well as its name. For example 'core.BasicClass'.
	 *
	 * <p>The search starts on the package returned by the {@link Cache#getRoot} method
