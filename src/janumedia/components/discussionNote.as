...
import UI.controls.*;
class janumedia.components.discussionNote extends MovieClip {
	var bg:MovieClip;
	var history;
	var isFocus:Boolean;
	var so:SharedObject;
	...
	function onSync(list:Array) {
		for (var i = 0; i<list.length; i++) {
			...
			if (list[i].name == "sharedDiscussionNotes") {
				switch (list[i].code) {
				case "delete" :
					updateNote("");
					break;
