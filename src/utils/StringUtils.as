/**
* class utils.string - AS2.0 [ 08/07/2004 ]
*
...
*               md5( s:String [, utils.string.b64pad:String [, utils.string.chrsz:Number ] ] ):String
*
* NOTE: you can read this methods list using utils.string.methodsList();
...
class utils.string {
	...
	*/
	public static function methodsList():Void {
		...
		trace( "md5( s:String [, utils.string.b64pad:String [, utils.string.chrsz:Number ] ] ):String" );
	...
	*/
	static public function trim( s:String ):String {
		return utils.string.ltrim( utils.string.rtrim( s ) );
	}
	...
	*/
	static public function rpos( s:String, src:String ):Object {
		return utils.string.__rpos( s, src, false );
	}
	...
	*/
	static public function ripos( s:String, src:String ):Object {
		return utils.string.__rpos( s, src, true );
	}
	...
	*/
	static public function pos( s:String, src:String, ofs:Number ):Object {
		return utils.string.__pos( s, src, ofs, false );
	}
	...
	*/
	static public function ipos( s:String, src:String, ofs:Number ):Object {
		return utils.string.__pos( s, src, ofs, true );
	}
	...
	*/
	static public function word_count( s:String ):Array {
		...
		for( var a:Number = 0; a < sNew.length; a++ ) {
			var nowS:String = utils.string.trim( sNew[a] );
			if( nowS != "" ) {
				tWords.push( sNew[a] );
			}
		}
	...
	*/
	static public function pad( s:String, p:Number, toAd:String, t:String ):String {
		if( s.length < p ) {
			...
			var repeated:String = utils.string.repeat( toAd, Math.ceil( p / toAd.length ) );
			if( t == "PAD_RIGHT" ) {
	...
	*/
	static public function ucwords( s:String ):String {
		var sNew:Array = s.split( " " );
		for( var a:Number = 0; a < sNew.length; a++ ) {
			sNew[a] = utils.string.ucfirst( sNew[a] );
		}
		return sNew.join( " " );
	}
	...
	*/
	static public function strip_tags( s:String, allow:Object ):String {
		...
		var removeTags:Function = function( s:String, allowable:Array, allow:Object ):String {
			...
			for( var a:Number = 0; a < sNew.length; a++ ) {
				if( utils.string.pos( sNew[a], ">" ) !== false ) {
					var htmlTag:String = "<" + sNew[a].substr( 0, utils.string.pos( sNew[a], ">" ) ) + ">";
					if( htmlTag != "<>" ) {
						if( ( allowable.length != undefined && in_array( allowable, htmlTag ) == false ) || allow == undefined ) {
							sNew[a] = sNew[a].substr( utils.string.pos( sNew[a], ">" ) + 1, sNew[a].length );
							modified = true;
						}
						else {
							sNew[a] = "<" + sNew[a];
	...
	*/
	static public function md5( s:String, b64pad:String, chrsz:Number ):String {
		if( b64pad != undefined ) {
			utils.string.b64pad = b64pad;
		}
		if( chrsz != undefined && chrsz == 8 || chrsz == 16 ) {
			utils.string.chrsz = chrsz;
		}
		return utils.string.hex_md5( s );
	}
	...
	*/
	private static function hex_md5( s:String ):String {
		return utils.string.binl2hex(core_md5(str2binl(s), s.length*utils.string.chrsz));
	}
	private static function b64_md5( s:String ):String {
		return utils.string.binl2b64(core_md5(str2binl(s), s.length*utils.string.chrsz));
	}
	private static function str_md5( s:String ):String {
		return utils.string.binl2str(core_md5(str2binl(s), s.length*utils.string.chrsz));
	}
	private static function hex_hmac_md5( key:String, data:String ):String {
		return utils.string.binl2hex(core_hmac_md5(key, data));
	}
	private static function b64_hmac_md5( key:String, data:String ):String {
		return utils.string.binl2b64(core_hmac_md5(key, data));
	}
	private static function str_hmac_md5( key:String, data:String ):String {
		return utils.string.binl2str(core_hmac_md5(key, data));
	}
	...
	*/
	private static function core_hmac_md5( key:String, data:String ):Array {
		...
		if( bkey.length > 16 ) {
			bkey = core_md5(bkey, key.length*utils.string.chrsz);
		}
		...
		var hash:Array = new Array( core_md5( ipad.concat( str2binl( data  )), 512 + data.length*utils.string.chrsz ) );
		return utils.string.core_md5(opad.concat(hash), 512+128);
	}
	...
	* If utils.string.chrsz is ASCII, characters >255 have their hi-byte silently ignored.
	*/
	private static function str2binl( str:String ):Array {
		var bin:Array = new Array();
		var mask:Number = ( 1 << utils.string.chrsz ) - 1;
		for( var i:Number = 0; i < str.length * utils.string.chrsz; i += utils.string.chrsz ) {
			bin[i >> 5] |= (str.charCodeAt(i/utils.string.chrsz) & mask) << (i%32);
		}
		return bin;
	}
	...
	*/
	private static function binl2str( bin:Array ):String {
		...
		var mask:Number = ( 1 << utils.string.chrsz )-1;
		for( var i:Number = 0; i < bin.length * 32; i += utils.string.chrsz ) {
