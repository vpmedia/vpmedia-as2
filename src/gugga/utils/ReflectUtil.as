import gugga.utils.MemberNamesClassification;
/*
...
class gugga.utils.ReflectUtil {
	...
	public static var METHOD_NAME_KEY : String = "__as2lib__methodName";
	...
	public static function getMethodNameByInstance(method:Function, instance):String {
		var r:String = method.__as2lib__methodName;
		if (r != null) return r;
	...
	{
		var classification : MemberNamesClassification = getMemberNamesClassification(instance);
		return classification.Variables;
	}
	
	public static function getVariableNames2(instance):Array {
	...
	public static function getGetterNames(instance) : Array 
	{
		var classification : MemberNamesClassification = getMemberNamesClassification(instance);
		return classification.Getters;
	}

	public static function getSetterNames(instance) : Array 
	{
		var classification : MemberNamesClassification = getMemberNamesClassification(instance);
		return classification.Setters;
	}

	public static function getAccessorNames(instance) : Array 
	{
		var classification : MemberNamesClassification = getMemberNamesClassification(instance);
		return classification.Accessors;
	}

	public static function getMethodAndAccessorNames(instance) : Array 
	{
		var classification : MemberNamesClassification = getMemberNamesClassification(instance);
		return classification.MethodsAndAccessors;
	}

	public static function getMethodNames(instance) : Array 
	{
		var classification : MemberNamesClassification = getMemberNamesClassification(instance);
		return classification.Methods;
	}

	public static function getMemberNamesClassification(instance) : MemberNamesClassification 
	{
		var classification : MemberNamesClassification = new MemberNamesClassification();		
		var memberNamesList : Array = getMemberNamesList(instance);
		
		var memberName : String;
		for(var i = 0; i < memberNamesList.length; i++)
		{
			memberName = memberNamesList[i];
			
			if (typeof(instance[memberName]) != "function")
			{
				classification.Variables.push(memberName);
			}
			else
			{
				if(memberName.indexOf("__get__") == 0)
				{
					classification.Getters.push(memberName);
				}
				else if(memberName.indexOf("__set__") == 0)
				{
					classification.Setters.push(memberName);
				}
				else
				{
					classification.Methods.push(memberName);
				}
			}
		}
	...
	public static function getMemberNamesList(instance) : Array 
	{
		getMethodNameByInstance222(instance.constructor, instance);
	...
	//These are enabling the magic of reflection :) ---------------------------------------------
	private static function getMethodNameByInstance222(method:Function, instance):String {
		if (instance.__constructor__) {
			if (instance.__constructor__["prototype"] == instance.__proto__) {
				return getMethodNameByType222(method, instance.__constructor__);
			}
		}
		if (instance.constructor) {
			if (instance.constructor["prototype"] == instance.__proto__) {
				return getMethodNameByType222(method, instance.constructor);
			}
		}
		return getMethodNameByPrototype222(method, instance.__proto__);
	}
	
	
	private static function getMethodNameByType222(method:Function, type:Function):String {
		var m:String = getMethodNameByPrototype222(method, type.prototype);
		if (m != null) return m;
		return getMethodNameByObject222(method, type);
	}
	

	private static function getMethodNameByPrototype222(m:Function, p):String {
		while (p) {
			var n:String = getMethodNameByObject222(m, p);
			if (n != null) return n;
			p = p.__proto__;
		}
		return null;
	...
	private static function getMethodNameByObject222(m:Function, o):String {
		var s:Function = _global.ASSetPropFlags;
		s(o, null, 0, true);
		for (var n:String in o) {
			try {
