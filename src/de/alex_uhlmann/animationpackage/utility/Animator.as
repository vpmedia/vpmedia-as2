...
* 			The tween engine used is either Macromedia's mx.effects.Tween or de.alex_uhlmann.animationpackage.utility.FrameTween 
	...
	*/
	...
	public function stopMe(Void):Void {		
		if(this.caller.getTweenMode() == AnimationCore.INTERVAL) {
			...
			Tween.RemoveTweenAt(this.myTween.ID);
		} else if(this.caller.getTweenMode() == AnimationCore.FRAMES) {
			this.elapsedDuration = this.computeElapsedDuration();			
			FrameTween.RemoveTweenAt(this.myTween.ID);
		}
		deleteAnimation();
		this.stopped = true;
	}
	...
	public function pauseMe(Void):Void {	
		if(this.caller.getTweenMode() == AnimationCore.INTERVAL) {			
			...
			this.elapsedDuration = this.computeElapsedDuration();
			delete Tween.ActiveTweens[this.myTween.ID];
		} else if(this.caller.getTweenMode() == AnimationCore.FRAMES) {
			this.startPause = FrameBasedInterval.frame;
			this.elapsedDuration = this.computeElapsedDuration();
			delete FrameTween.ActiveTweens[this.myTween.ID];
		}
	}
	
	public function resumeMe(Void):Void {		
		if(this.caller.getTweenMode() == AnimationCore.INTERVAL) {
			this.pausedTime = getTimer() - this.startPause;
			this.myTween.startTime += this.pausedTime;			
			Tween.ActiveTweens[this.myTween.ID] = this.myTween;
		} else if(this.caller.getTweenMode() == AnimationCore.FRAMES) {
			this.pausedTime = FrameBasedInterval.frame - this.startPause;
			this.myTween.startTime += this.pausedTime;
			FrameTween.ActiveTweens[this.myTween.ID] = this.myTween;			
		}
	}
