...
class de.alex_uhlmann.animationpackage.animation.AnimationCore 
	...
	*/
	public static function pauseAll(Void):Void {
		//time based tweening.
		clearInterval(Tween.IntervalToken);
		AnimationCore.startTime = getTimer();
	...
	*/
	public static function resumeAll(Void):Void {		
		//time based tweening.
		Tween.IntervalToken = setInterval(Tween.Dispatcher, "DispatchTweens", Tween.Interval);
		var aT:Array = Tween.ActiveTweens;
		var len:Number = aT.length;
		var i:Number;
		for (i=0; i<len; i++) {
			aT[i].startTime += (getTimer() - AnimationCore.startTime);				
		}
		//frame based tweening.
